...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/medium'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Medium) {
        return item.id;
    }
    registerChangeInMediums() {
        this.eventSubscriber = this.eventManager.subscribe('mediumListModification', (response) => this.loadAll());
    }
