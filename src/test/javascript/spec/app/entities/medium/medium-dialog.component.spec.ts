/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject, fakeAsync, tick } from '@angular/core/testing';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';
import { Observable } from 'rxjs/Observable';
import { JhiEventManager } from 'ng-jhipster';

import { HipmediadbTestModule } from '../../../test.module';
import { MediumDialogComponent } from '../../../../../../main/webapp/app/entities/medium/medium-dialog.component';
import { MediumService } from '../../../../../../main/webapp/app/entities/medium/medium.service';
import { Medium } from '../../../../../../main/webapp/app/entities/medium/medium.model';
import { MediumtypService } from '../../../../../../main/webapp/app/entities/mediumtyp';

describe('Component Tests', () => {

    describe('Medium Management Dialog Component', () => {
        let comp: MediumDialogComponent;
        let fixture: ComponentFixture<MediumDialogComponent>;
        let service: MediumService;
        let mockEventManager: any;
        let mockActiveModal: any;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [HipmediadbTestModule],
                declarations: [MediumDialogComponent],
                providers: [
                    MediumtypService,
                    MediumService
                ]
            })
            .overrideTemplate(MediumDialogComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(MediumDialogComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(MediumService);
            mockEventManager = fixture.debugElement.injector.get(JhiEventManager);
            mockActiveModal = fixture.debugElement.injector.get(NgbActiveModal);
        });

        describe('save', () => {
            it('Should call update service on save for existing entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Medium(123);
                        spyOn(service, 'update').and.returnValue(Observable.of(entity));
                        comp.medium = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.update).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'mediumListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );

            it('Should call create service on save for new entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Medium();
                        spyOn(service, 'create').and.returnValue(Observable.of(entity));
                        comp.medium = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.create).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'mediumListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );
        });
    });

});
